...
/**
 * 适用于 2个页面的viewPager 指示器
 */
public class Pager2TabView extends HorizontalScrollView {
    ...
    // private int indicatorColor = 0xFFfd5224;
    // private int underlineColor = 0x1A000000;
    private int underlineColor = 0xFFfd5224;
    ...
    private int tabBackgroundResId = R.drawable.base_sel_viewpager_tab;

    private Locale locale;

    public Pager2TabView(Context context) {
        this(context, null);
    }


    public Pager2TabView(Context context, AttributeSet attrs) {
    ...
    private void addTab(final int position, View tab) {
        ...
        tab.setPadding(tabPadding, 9, tabPadding, 9);
        tabsContainer
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int roundoffset = 60;
        RectF outerRect = new RectF(lineLeft + div, div, lineRight - div,
                height - div);

        canvas.drawRoundRect(outerRect, outerRect.height() / 2, outerRect.height() / 2, rectPaint);
        if (currentPosition == 0 && currentPositionOffset <= 0.5F) {
            canvas.drawRect(lineRight - div - roundoffset, div,
                    lineRight - div, height - div, rectPaint);
        } else if (currentPosition == 0 && currentPositionOffset > 0.5F) {
            canvas.drawRect(lineLeft + div, div, lineLeft + div + roundoffset,
                    height - div, rectPaint);
        } else if (currentPosition == tabCount - 1) {
            canvas.drawRect(lineLeft + div, div, lineLeft + div + roundoffset,
                    height - div, rectPaint);
        } else {
            canvas.drawRect(lineLeft + div, div, lineRight - div, height - div,
                    rectPaint);
        }
    ...
    private class PageListener implements OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            ...
            if (onPageSeletedListener != null) {
                onPageSeletedListener.onPageSelete(selectedPosition);
            }
            updateTabStyles();
            if (delegatePageListener != null) {
    ...
    private IOnPageSeletedListener onPageSeletedListener;


    public void setOnPageSeletedListener(
            IOnPageSeletedListener onPageSeletedListener) {
        this.onPageSeletedListener = onPageSeletedListener;
    }

    public interface IOnPageSeletedListener {
        void onPageSelete(int postion);
    }
